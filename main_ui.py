...
def save_metadata():
    ...
    if image_path and (overwrite or output_path):
        ...
        steps = guidance_entry.get().strip()
        sampler = sampler_var.get().strip()
        seed = seed_entry.get().strip()
        clip_skip = clip_skip_entry.get().strip()
        ...
        new_metadata = (
            ...
            f"Steps: {steps}, Sampler: {sampler}, Schedule type: Karras, CFG scale: {clip_skip}, Seed: {seed}, "
...
guidance_label = tk.Label(right_frame, text="Steps:", bg="#e6f2ff", font=("Arial", 12))
guidance_label.pack(pady=5, anchor="w")
guidance_entry = tk.Entry(right_frame, width=50, bg="#f8f8ff")
guidance_entry.insert(0, "20")  # Default value for steps
...
guidance_entry.bind("<KeyRelease>", lambda event: validate_integer_input(event, "Steps"))
...
clip_skip_label = tk.Label(right_frame, text="Clip Skip:", bg="#e6f2ff", font=("Arial", 12))
clip_skip_label.pack(pady=5, anchor="w")
clip_skip_entry = tk.Entry(right_frame, width=50, bg="#f8f8ff")
clip_skip_entry.insert(0, "2")  # Default value for clip skip
clip_skip_entry.pack(padx=10, pady=5, fill=tk.X)
clip_skip_entry.bind("<KeyRelease>", lambda event: update_metadata_preview())  # Update on key release
clip_skip_entry.bind("<KeyRelease>", lambda event: validate_integer_input(event, "Clip Skip"))
